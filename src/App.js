...
function ScanVLogoMark({ size = LOGO_SIZE.sm, center = false, linkToApp = false }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: center ? 'center' : 'flex-start', flexShrink: 0 }}>
      {logoBody}
    </div>
  );
}
...
const TWO_WHEELER_SVCS = [
  ...
  { id:'tw-polish', parent:'two-wheeler', theme:'care', icon:'🪩', img:'/services/two-wheeler/polish.png', name:'Bike Polish & Wax', sub:'Body wax · chrome shine · chain lube · 45 min', unit:'visit', mrp:11900, price:discPaise(11900), cash:false,
    desc:'ScanV bike polish & wax — premium body wax, chrome buff, tyre dressing, and chain lube for a glossy showroom finish at your parking spot.',
    features:['Premium body wax','Chrome & plastic polish','Tyre dressing','Chain lube included','Same-day booking'], turnaround:'Same day', rating:'4.8 ⭐', bookings:'1,700+' },
...
const FOUR_WHEELER_SVCS = [
  ...
  { id:'fw-sanitize', parent:'four-wheeler', theme:'care', icon:'🧴', img:'/services/four-wheeler/sanitize.png', name:'Interior Sanitization', sub:'Ozone · AC disinfect · fabric spray · 1 hr', unit:'visit', mrp:24900, price:discPaise(24900), cash:false,
    desc:'ScanV interior sanitization — ozone treatment, AC vent disinfect, fabric and touch-point spray for a fresh, germ-free cabin.',
...
function CloudCourseFeeAmount({ amountPaise, subColor = C.dim }) {
  return (
    <div style={{ textAlign: 'right' }}>
      <span>₹{fmtRs(amountPaise)}</span>
      <div style={{ fontSize: 9, color: subColor, fontWeight: 500, marginTop: 1, lineHeight: 1.2 }}>(+ GST Platform)</div>
    </div>
  );
}
...
async function invokeSendOtp(mobile, termsAcceptedAt = null) {
  const norm = mobile.startsWith('+') ? mobile : `+91${mobile.replace(/\D/g,'').slice(-10)}`;
  const body = { mobile: norm };
  if (termsAcceptedAt) {
    body.terms_accepted = true;
    body.terms_accepted_at = termsAcceptedAt;
  }
  ...
  const sendLoginWA = async () => {
    ...
    const ok = await startWAVerify(`+91${mobile}`, () => loginProfile(true), {
      ...
      onUnavailable: async (msg) => {
        ...
        try {
          await invokeSendOtp(`+91${mobile}`, termsAcceptedAt);
          setOtpSent(true);
          setOtpTargetMobile(mobile);
        } catch (_) {}
      },
        ...
        <div className="scanv-scroll-body" style={{...BROWSE_SCROLL_BODY,padding:'14px 16px 24px'}}>
          ...
          {browseCloudBook && (
            <div style={{...S.card(),marginBottom:12,padding:14,fontSize:13}}>
              <div style={{display:'flex',justifyContent:'space-between',padding:'6px 0',color:C.sub}}><span>Course fee</span><CloudCourseFeeAmount amountPaise={browseCourseFee} subColor={C.dim} /></div>
          ...
          <div style={S.card({marginBottom:14,padding:'12px 14px'})}>
            {browseCloudBook ? (
              <>
                <div style={{ display: 'flex', justifyContent: 'space-between', padding: '5px 0', color: C.sub, fontSize: 14 }}>
                  <span>Course fee</span>
                  <CloudCourseFeeAmount amountPaise={browseCourseFee} />
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', padding: '5px 0', borderTop: `1px solid ${C.bdr}`, fontWeight: 800, color: C.acc, fontSize: 16 }}>
                  <span>Pay now</span><span>₹{(total/100).toLocaleString('en-IN')}</span>
                </div>
...
function RegistrationFlow({ onComplete, prefill, onGoToLogin }) {
  ...
  const sendOTP = async () => {
    ...
    try {
      ...
      setWaToken('');
      const sent = await invokeSendOtp(mob, termsAcceptedAt);
...
function BottomNav() {
  ...
  const goTab=(id)=>{
    if (id==='services'||id==='top-rated') setActiveSvc(null);
    setScreen(id);
  };
  return (
...
const SVC_DETAIL = {
  ...
  beauty:   { desc:'Makeup, salon, and grooming at home — facial, mehendi, and more from verified partners · 10 beauty services · 25% off.', features:['Women\'s & men\'s haircut','Beard trim & shave','Threading & waxing','Party & bridal makeup','Mehendi at home'], turnaround:'Same day', rating:'4.8 ⭐', bookings:'4,200+' },
...
function TopRatedScreen() {
  ...
  const openSvc = (svc) => {
    ...
    setActiveSvc(svc);
    setScreen(SUB_CATEGORIES[svc.id] ? 'services' : 'book');
  };
...
function BookScreen() {
  ...
  const sendBookOTP=async(resend=false)=>{
    ...
    try{
      ...
      const sent = await invokeSendOtp(mob, bookTermsAcceptedAt);
    ...
    <div style={{flex:1,overflowY:'auto',fontFamily:FF}}>
      ...
      <div style={{padding:'8px 16px 40px'}}>
        {step===1&&<>
          <div style={{...S.card(),marginBottom:20,padding:0,overflow:'hidden'}}>
            ...
            <div style={{padding:16}}>
              ...
              {isCloudBook ? (
                <>
                  <div style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', color: C.txt, fontSize: 15 }}>
                    <span>Course fee</span>
                    <CloudCourseFeeAmount amountPaise={courseFee} />
                  </div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderTop: `1px solid ${C.bdr}`, fontWeight: 700, color: C.acc, fontSize: 17 }}>
                    <span>Pay now (Razorpay)</span><span>₹{fmtRs(total)}</span>
                  </div>
        ...
        {step===2&&!skipVerify&&<>
          ...
          <TermsAcceptanceField accepted={bookTermsAccepted} onAccept={acceptBookTerms} onRevoke={revokeBookTerms} C={C} BDR={BDR} />
        ...
        {step===payStep&&<>
          ...
          {isCloudBook && (
            <div style={{...S.card(),marginBottom:12,padding:14,fontSize:13}}>
              <div style={{display:'flex',justifyContent:'space-between',padding:'6px 0',color:C.sub}}><span>Course fee</span><CloudCourseFeeAmount amountPaise={courseFee} subColor={C.dim} /></div>
              <div style={{display:'flex',justifyContent:'space-between',padding:'6px 0',color:C.sub}}><span>ScanV share + fees</span><span style={{fontWeight:700,color:C.acc}}>₹{fmtRs(total)}</span></div>
              ...
              {shown.map((r) => {
                ...
                return (
                  <tr key={r.service_id} style={{ opacity: st === 'active' ? 1 : 0.72 }}>
                    ...
                    <td style={td}>
                      <select value={r.top_rated ?? 0} onChange={e=>updateRow(idx,'top_rated',e.target.value)} style={{ ...inp, width:96 }}>
  ...
  const pages = {
      ...
      content: (
        <>
          <div style={{background:`${C.grn}22`,border:`1px solid ${C.grn}44`,borderRadius:10,padding:14,marginBottom:24}}>
            <p style={{margin:0,color:C.grn,fontSize:13}}>🔒 Data stored in India (AWS Mumbai) · DPDP Act 2023 compliant · Applies to all ScanV joiners · We never sell personal data</p>
          </div>
          {[
      ...
      content: (
        <>
          ...
          {[
            ...
            ['Severability & Entire Agreement','If any provision is held invalid, the remainder stays in effect. These Terms, Privacy Policy, DPDP summary, Refund Policy, Payment Policy, and Partner Terms & Conditions (for partners/vendors/service providers at /partner-terms) together form the entire agreement regarding ScanV.'],
    ...
    refund: {
      ...
      content: (
        <>
          <div style={{background:`${C.gold}22`,border:`1px solid ${C.gold}44`,borderRadius:10,padding:14,marginBottom:24}}>
            <p style={{margin:0,color:C.gold,fontSize:13}}>⚠️ This Refund Policy applies to all ScanV joiners. {SCANV_LIABILITY_DISCLAIMER} Refunds are processed only as stated below — DCore Global Corporation is not liable for service-provider, vendor, or training outcomes beyond these rules.</p>
          </div>
          {[
          ...
          {[
            ['What DCore Global Corporation Refunds','In-app client-user cancellation (70% of total paid) · DCore cancels due to unavailable service provider/vendor · Technical error causing incorrect charge · Duplicate payment · Payment processed but no booking or enrolment confirmed'],
            ['What DCore Global Corporation Does Not Refund','The 30% cancellation fee on client-initiated cancels · SGR/cloud fees after form submission (unless programme cancelled by DCore) · Service quality disputes (client vs service provider) · Vendor product disputes · Candidate placement or training outcomes · User no-show · Change of mind after service started · Cash payments to providers/vendors · Professional outcomes (legal, medical, cloud certification)'],
            ['Non-Refundable Categories','Legal consultations (once conducted) · Cloud training / SGR (once batch started or consultant assigned) · VIP appointments (deposit within 24hrs) · Food (once preparation started) · Healthcare (once consultation complete) · Vendor goods (once dispatched unless DCORE error)'],
            ...
            ['How to Request','App: Bookings or Track → Cancel booking · Cloud/SGR: email refunds@dcoreglobal.com with payment reference · Email: refunds@dcoreglobal.com with TXN-XXXXXXXX · Include joiner category (client user, candidate, vendor, service provider)'],
            ['Limitation of Liability',`${SCANV_LIABILITY_DISCLAIMER} Refund processing does not constitute admission of liability by ScanV or DCore Global Corporation.`],
          ].map(([h,b])=>(<div key={h} style={{marginBottom:20}}><div style={{color:C.txt,fontWeight:600,fontSize:14,marginBottom:6,paddingBottom:6,borderBottom:`1px solid ${C.bdr}`}}>{h}</div><p style={{color:C.sub,fontSize:13,lineHeight:1.7,margin:0}}>{b}</p></div>))}
        </>
      )
    },
