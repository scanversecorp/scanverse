...
function App() {
  ...
  // Route to legal pages if path matches exactly
  const _path = window.location.href.split(window.location.host)[1]?.split('?')[0]?.replace(/^\//, '') || '';
  if (_path === 'privacy') return <Boundary><style>{CSS}</style><LegalPage page="privacy"/></Boundary>;
  if (_path === 'terms') return <Boundary><style>{CSS}</style><LegalPage page="terms"/></Boundary>;
  if (_path === 'refund') return <Boundary><style>{CSS}</style><LegalPage page="refund"/></Boundary>;
  if (_path === 'payment') return <Boundary><style>{CSS}</style><LegalPage page="payment"/></Boundary>;
