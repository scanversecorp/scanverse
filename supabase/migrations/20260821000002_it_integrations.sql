...
INSERT INTO public.it_integrations (
  ...
  ('twilio', 'Twilio', '', 'https://console.twilio.com', 'https://api.twilio.com',
   'TWILIO_ACCOUNT_SID,TWILIO_AUTH_TOKEN', 'Account SID + TWILIO_AUTH_TOKEN — international SMS/voice last resort', 'send-otp voice/SMS · dispatch', 'vendor_enable_twilio', 'on', 40),
  ('whatsapp', 'MSG91 WhatsApp', '+91-9270194842', 'https://control.msg91.com/app/m/l/whatsapp', 'https://api.msg91.com/api/v5/whatsapp',
   'MSG91_AUTH_KEY,MSG91_WHATSAPP_INTEGRATED_NUMBER', 'MSG91 auth + integrated number 919270194842 — WhatsApp OTP backup channel', 'whatsapp-verify · booking OTP', 'vendor_enable_whatsapp', 'on', 50),
  ('razorpay', 'Razorpay', '+91-120-445-5666', 'https://dashboard.razorpay.com', 'https://api.razorpay.com/v1',
   'RAZORPAY_KEY_ID,RAZORPAY_KEY_SECRET', 'Key ID + secret — payment links, webhooks, Route transfers', 'razorpay-payment · vendor payouts', 'vendor_enable_razorpay', 'on', 60),
  ...
  ('digio', 'Digio', '+91-888-070-0700', 'https://www.digio.in', 'https://api.digio.in',
   'DIGIO_CLIENT_ID,DIGIO_CLIENT_SECRET', 'Client ID + secret — vendor Aadhaar eKYC', 'vendor-onboard ekyc-aadhaar', NULL, 'on', 140),
  ('pan_verify', 'PAN Verification API', '', '', '',
   'PAN_VERIFY_API_KEY', 'Third-party PAN lookup — vendor onboarding document check', 'vendor-onboard PAN verify step', NULL, 'on', 145),
  ('resend', 'Resend', '', 'https://resend.com/emails', 'https://api.resend.com',
   'RESEND_API_KEY,SUPPORT_EMAIL_FROM', 'Bearer token + from address — transactional email', 'support-tickets · booking confirmation · health reports', NULL, 'on', 150),
  ('cloudflare_email', 'Cloudflare Email', '', 'https://dash.cloudflare.com', 'https://api.cloudflare.com/client/v4',
   'CLOUDFLARE_API_TOKEN,SUPPORT_EMAIL_FROM', 'API token + from address — email sending fallback', 'notify.ts email when Resend unavailable', NULL, 'on', 160),
  ('cloudflare_cdn', 'Cloudflare CDN & DNS', '', 'https://dash.cloudflare.com', 'https://api.cloudflare.com/client/v4',
   'CLOUDFLARE_API_TOKEN', 'API token — DNS, CDN cache, WAF, Email Routing (zone getscanv.com)', 'getscanv.com edge · CF-Ray · cache · email routing scripts', NULL, 'on', 165),
  ...
  ('github', 'GitHub', '', 'https://github.com/scanversecorp/scanverse', 'https://api.github.com',
   '', 'Repo + Actions secrets — Vercel Git deploy, workflow PAT for Instagram cron', 'Source control · CI · instagram-daily workflow', NULL, 'on', 185),
  ('meta_instagram', 'Meta Instagram', '', 'https://developers.facebook.com', 'https://graph.facebook.com',
   '', 'Vercel/GitHub env — META_PAGE_ACCESS_TOKEN + META_IG_USER_ID', 'Daily @scanvapp posts · instagram_daily_post.mjs', NULL, 'on', 190)
